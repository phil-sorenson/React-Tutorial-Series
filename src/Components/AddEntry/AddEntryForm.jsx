...
//⭐ In order to utilize onChange & input event, we need to update the state-'weight' variable to = input event (Do it through setWeight) --->
    ...
    return ( 
        ...
        <form onSubmit= {handleSubmit}>
            ...
            {/* every letter typed into an input box also causes an input EVENT 👇 to be triggered (Allows us to access the value inputed from browser) */}
            <input type= 'number' value= {weight} onChange={(event)=> setWeight(parseFloat(event.target.value))}/> {/*📝 'parseFloat' used to change the weight output from 'string' to int*/}
            <label>Date</label>
